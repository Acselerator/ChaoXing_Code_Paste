...
- **追加写入**：粘贴内容默认追加到已有答案末尾，避免覆盖已输入内容。
...
本脚本优先通过`editor.replaceRange()`在文末追加内容，回退时使用`editor.setValue(editor.getValue() + content)`，从而绕过粘贴检测且不覆盖原有内容。
...
本脚本通过直接调用`editor.setContent(htmlContent, true)`追加内容，完全绕过`beforepaste`事件且不覆盖原有答案。
...
7. **保存提交**：粘贴或图片插入成功后，记得点击页面的"暂时保存"或"提交"按钮。
...
### v1.3.0 (2026-06-11)
- 新增追加写入模式，CodeMirror和UEditor粘贴不再覆盖已有内容
